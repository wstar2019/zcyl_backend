...
<body>
	<div class="page-header">
		<ul class="navi">
			...
			<li><a target="mainFrame" href="sa/listArticle">商品管理</a><span> ≫ </span></li>
			<c:if test="${empty art}">
				<li><a target="mainFrame" href="sa/editArticle/?ID=${art.ID}">上架新商品</a></li>
			</c:if>
			<c:if test="${not empty art}">
				<li><a target="mainFrame" href="sa/editArticle/?ID=${art.ID}">编辑</a></li>
			</c:if>
	...
	<div class="page-content" style="padding: 10px;">
		<div class="container-fluid">
			<form  action="sa/saveArt" name="articleForm" id="articleForm" method="post" enctype="multipart/form-data">
				<div class="row input-controls">
					...
					<div class="col-md-12">
						...
						<div class="form-group" id="saOption">
							...
							<div class="col-sm-10">
								...
								<div class="row">
									<div class="col-sm-4">
										<select class="form-control" name="SA_OPTION" id="SA_OPTION">
									...
									<div class="col-sm-7">
										<select class="form-control" name="SA_OPTION_VALUE" id="SA_OPTION_VALUE">
											<option value="">请选择</option>
										</select>
									</div>
									<div class="col-sm-1">
...
function selectClass(){
	...
	if(cid != ''){
		$('input:radio[name="sacGroup"][value="' + cid + '"]').prop('checked', true);
	}
	$('#classModal').modal("show");
}
function confirmClass(){
	var selCID = $('input:radio[name="sacGroup"]:checked').val();
...
function addOption(){
	var opt_idx = $("#SA_OPTION")[0].selectedIndex;
	var opt_label =$("#SA_OPTION")[0].options[opt_idx].label;	
	var opt_id = $("#SA_OPTION").val().toString();
	var val_idx = $("#SA_OPTION_VALUE")[0].selectedIndex;
	var val_label =$("#SA_OPTION_VALUE")[0].options[val_idx].label;
	var val_id = $("#SA_OPTION_VALUE").val();
	...
	if(opt_id != "" && val_id != ""){
		...
		$('#SA_OPTION option:eq(' + opt_idx + ')').remove();
		$('#SA_OPTION_VALUE option:eq(' + val_idx + ')').remove();
...
$('#SA_OPTION').change(function() {
	...
	else
	{
		$.ajax({
			type: "POST",
			url: 'sa/findOptValByMid',
			data: { MID : option_id },
			dataType:'json',
			success: function (res) {
				$("#SA_OPTION_VALUE>option").remove();
				$('#SA_OPTION_VALUE').append( "<option value=''>请选择</option>" );
				if ("success" == res.result) {
					var _v = res.data;
					for(var i = 0; i < _v.length; i++){
						$('#SA_OPTION_VALUE').append( "<option value='" + _v[i].ID + "'>" + _v[i].NAME + "</option>" );
					}
				} else {
					alert("FAILED");
				}
...
$('#MAIN_IMAGE_PREVIEW').attr('src',"static/uploadFiles/uploadImgs/sa/" + main_image);
...
for(var i = 0; i < detail_images.length; i++ ){
	...
	else
	{
		if(detail_images[i].trim() != "")
			$('#preImage' +(i + 1)).attr('src',"static/uploadFiles/uploadImgs/sa/" + detail_images[i]);
	}
}
...
function saveArticle(_ID){
	...
	//if(checkValidity()){
	    ...
	    fd.append("ID",_ID);
   		...
   		fd.append("SA_CLASS", $("#A_CLASS_ID").val());
		...
		$.ajax({
			...
			success: function (data) {
				if ("success" == data.result) {
					$("#articleForm").attr( "action", "sa/listArticle/?" + $("#articleForm").serialize());
					$("#articleForm").submit();
				} else {
					alert("FAILED");
				}
...
$(function() {
	...
	function makeTree(nodes){
		...
		for(var i = 0; i < nodes.length; i++){
			...
			var n = {
					value: el.id,
					expanded: true,
					html: "<input type='radio' name='sacGroup' value='" 
	...
	var source = makeTree(mnObj);
     ...
     $('#jqxTree').on('itemClick', function (event)
	 {
	     ...
	     $('input:radio[name=sacGroup]').each(function() {
    	    var d = $(this)[0].value;
    	    if(d == value){

...
<body class="no-skin">
	<div class="page-header">
		<ul class="navi">
			...
			<li><a target="mainFrame" href="sa/listKind">分类管理</a></li>
		</ul>
	</div>
	
	<div class="page-content">
		<form action="sa/listKind" method="post" name="ClassListForm" id="ClassListForm">	
						...
						<c:when test="${not empty ClassList}">
							<c:forEach items="${ClassList}" var="cla" varStatus="vs">
								<tr tr_id='${cla.CLASS_ID }' expanded="false"
									...
									<td class="center">
										<img alt="" src="<%=basePath%>uploadFiles/uploadImgs/sa/${cla.CLASS_IMG_URL}" style="height: 45px;">																
									</td>	
									<td class="center">
										<div class="btn-group">
											<%-- <c:if test="${QX.add == 1 }"> --%>
												<a class="btn btn-minier btn-white btn_m_add" data-toggle="modal" style="cursor:pointer; margin-right: 20px;"
														onclick="addChildClass('${cla.CLASS_ID}','${cla.CLASS_NAME}')">添加下级分类</a>
											<%-- </c:if>
											<c:if test="${QX.edit == 1 }"> --%>
												<a class="btn btn-minier btn-white btn_m_edit" style="cursor:pointer; margin-right: 20px;"
											...
											<%-- </c:if>
											<c:if test="${QX.del == 1 }"> --%>
												<a class="btn btn-minier btn-white btn_m_delete" onclick="deleteClass('${cla.CLASS_ID}', '${cla.CLASS_NAME}')">删除</a>
											<%-- </c:if>			 --%>														
										</div>
									</td>
				...
				<div class="modal-content">
					<div class="modal-header mymodal-header">
	      			...
	      			<div class="modal-body">
	      				<form action="sa/saveClass" method="post" name="NewClassForm"
							id="NewClassForm" enctype="multipart/form-data">
      						...
      						<div class="row">
							    ...
							    <div class=" col-sm-7" style="text-align: left;">
							        <input type="file" id="class_photo" name="CLASS_PHOTO" style="display: none;" onchange="uploadFile(this)" />
							        <a onclick="fileType('class_photo')" class="btn btn-mini btn-success" style="height: 32px; align: center;">选择文件</a>
							        
							        <img id="class_photo_preview" alt="" src="" style="height: 45px;">
							        
							        <input type="text" name="CLASS_PHOTO_NAME" id="class_photo_name"
										accept="image/*" maxlength="32"
							        	placeholder="来选择任何文件" title="头像图片" style="width: 200px; display:none;" readonly/>
							    </div>
      						</div>
      						<div class="row">
      							<div class="col-sm-3">
		...
		<div class="modal fade" id="DeleteModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">						
	  		<div class="modal-dialog" role="document" style="width:400px">
	  			<div class="modal-content">
	      			...
	      			<div class="modal-body">
	      				<form action="sa/deleteClass" name="DeleteForm" id="DeleteForm" method="post" style="font-size: 19px;">
...
<script type="text/javascript">
	...
	function editClass(_id, _name, _pid, _visi, _lay, _show, _img, _cont){
		...
		$("#class_photo_preview").attr('src', "<%=basePath%>uploadFiles/uploadImgs/sa/" + _img);
	...
	function deleteConfirm(){
		$("#DeleteForm").attr("action", "sa/deleteClass");
		$("#DeleteForm").submit();
	...
	function saveClass() {
		...
		$("#NewClassForm").attr("action", "sa/saveClass");
		$("#NewClassForm").submit();
	}

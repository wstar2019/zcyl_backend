...
<body>
	<div class="page-header">
		<ul class="navi">
			...
			<li><a target="mainFrame" href="sa/listOption">选项管理</a></li>
		</ul>
	</div>
	...
	<div class="page-content" style="padding: 10px;">
		<div class="container-fluid">
			...
			<form action="sa/listOption" method="post" name="optionForm" id="optionForm">
									...
									<tr id="row_${option.ID}"><td class="center">${(page.currentPage -1) * page.showCount + numStds}</td>
										...
										<td class="center">
											<div class="btn-group">
												...
												<a class="btn btn-minier btn-white btn_m_child" target="mainFrame" href="sa/listOptVal/?MID=${option.ID}">选项值</a>
											</div>
										</td>
									</tr>
								</c:forEach>
...
function refresh(){
	...
	if(pageNum == '0'){
		$("#optionForm").attr("action", "sa/listOption/?" + $("#optionForm").serialize());
		$("#optionForm").submit();	
	}else{
...
function changeStateOption(_id, _n, _c, _s){
	...
	bootbox.confirm(options, function(result) {
		if(result) {
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>sa/changeStateOpt',
...
function delOption(_id, _name, _state){
	...
	bootbox.confirm(options, function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>sa/deleteOpt?ID=" + _id;

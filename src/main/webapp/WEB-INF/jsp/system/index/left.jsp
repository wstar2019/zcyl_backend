<div id="sidebar" class="sidebar responsive">
	<ul id='menu' class="nav nav-list">
		<c:forEach items="${menuList}" var="menu1">
			...
			<li id="lm${menu1.MENU_ID }">
				<a style="cursor: pointer;" class="dropdown-toggle">
					<img class="img " src="${menu1.MENU_ICON}">
						...
						<c:if test="${menu2.hasMenu && '1' == menu2.MENU_STATE}">
							<li class="" id="z${menu2.MENU_ID }">
								<a style="cursor: pointer;"
									<c:if test="${not empty menu2.MENU_URL && '#' != menu2.MENU_URL}"> target="mainFrame" href="${menu2.MENU_URL}" class="real-menu"</c:if>
										...
										<c:forEach items="${menu2.subMenu}" var="menu3">
											<c:if test="${menu3.hasMenu && '1' == menu3.MENU_STATE}">
												<li class="" id="m${menu3.MENU_ID }">
													<a style="cursor: pointer;"
														<c:if test="${not empty menu3.MENU_URL && '#' != menu3.MENU_URL}">
															target="mainFrame" href="${menu3.MENU_URL }" class="real-menu"
														</c:if>
														<c:if test="${'[]' != menu3.subMenu}"> class="dropdown-toggle"</c:if>
													>
																...
																<c:if test="${menu4.hasMenu && '1' == menu4.MENU_STATE}">
																	...
																	<c:if test="${'[]' != menu4.subMenu}">
																		<a style="cursor: pointer;" target="mainFrame"
																				onclick="siMenu('n${menu4.MENU_ID }','m${menu3.MENU_ID }','${menu4.MENU_NAME }',
																							'menu/otherlistMenu.do?MENU_ID=${menu4.MENU_ID }')">
																	</c:if>
																	<c:if test="${'[]' == menu4.subMenu}">
																		<a style="cursor: pointer;" target="mainFrame"
																		<c:if test="${not empty menu4.MENU_URL && '#' != menu4.MENU_URL}">
																			target="mainFrame" onclick="siMenu('n${menu4.MENU_ID }','m${menu3.MENU_ID }','${menu4.MENU_NAME }','${menu4.MENU_URL }')"
																		</c:if>>
																	</c:if>
																		<i class="${menu4.MENU_ICON == null ? 'menu-icon fa fa-leaf black' : menu4.MENU_ICON}"></i>
																		${menu4.MENU_NAME } </a> <b class="arrow"></b>
																	</li>
																</c:if>
															</c:forEach>
														</ul>

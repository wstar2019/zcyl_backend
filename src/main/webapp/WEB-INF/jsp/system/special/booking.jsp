...
<body>
	<div class="page-header">
		<ul class="navi">
			...
			<li><a target="mainFrame" href="sp/listBooking">订单管理</a><span> ≫ </span></li>
			<li>
				<c:if test="${pd.EDIT == true}">
					<a target="mainFrame" href="sp/booking/?ID=${pd.ID}&EDIT=true">编辑</a>
				</c:if>
				<c:if test="${pd.EDIT == false}">
					<a target="mainFrame" href="sp/booking/?ID=${pd.ID}&EDIT=false">编辑</a>
				</c:if>
			</li>
		</ul>
	</div>
	<!-- /section:basics/navbar.layout -->
				...
				<div class="page-content" style="padding: 10px;">
					<div class="container-fluid">
						<form class="b-info" action="sp/saveBooking" name="bookingForm" id="bookingForm" method="post" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">
									    ...
									    <div class="col-sm-10 control-value">${booking.booking_state}</div>
								  	</div>
								</div>
							</div>
							<div class="row">
										...
										<table id="simple-table" class="table table-hover" style="margin-top: 5px;">
											...
											<c:forEach items="${logs}" var="log" varStatus="vs">
												...
												<tr id="row_${log.id}"><td class="center">${numStds}</td>
													...
													<td class="center option-comment">${log.pre_state} -> ${log.cur_state}</td>
...
function addLogItem() {
	var strHtml = "<td class='center' style='width:100px !important;'>"
		...
		+ "<td class='center padding0 bl_state'><input type='text' class='form-control'/></td>"
...
function goListPage(){
	$("#bookingForm").attr( "action", "sp/listBooking");
	$("#bookingForm").submit();
}

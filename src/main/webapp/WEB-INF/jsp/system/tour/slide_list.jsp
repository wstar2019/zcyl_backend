...
<body>
	<div class="page-header">
		<ul class="navi">
			...
			<li><a target="mainFrame" href="tour/listSlide">横幅管理</a></li>
		</ul>
	</div>
	<div class="page-content" style="padding: 10px;">
		<div class="container-fluid">
			...
			<form action="tour/listSlide" method="post" name="tourSlideForm" id="tourSlideForm">
								...
								<c:forEach items="${slideList}" var="sl" varStatus="vs">
									...
									<tr id="row_${sl.id}">
										...
										<td class="center sl-name">
											<a class="btn_m_view_img" data-toggle="modal" data-target="#viewSlidingImage">
												<img style="width: 120px; height: 40px;cursor:pointer;" alt="" src="uploadFiles/uploadImgs/tour/${sl.name}">
											</a>	
										</td>
										<td class="center sl-desc">${sl.comment }</td>
										<td class="center sl-state">
											<a class="btn btn-minier btn-white btn_m_delete"
												onclick="changeStateSlide('${sl.id}', '${sl.state}');">
												${sl.state == 1 ? '隐藏' : '显示'}
											</a>
										</td>							
										<td class="center">
											<div class="btn-group">
												...
												<a class="btn btn-minier btn-white btn_m_delete"
													onclick="delSlide('${sl.id}','${sl.name}');">删除</a>
											</div>
										</td>
									</tr>
								</c:forEach>
			...
			<div class="modal fade" id="slideModal" tabindex="-1" role="dialog" aria-labelledby="slideModalLabel">						
		      				...
		      				<table>
		      					<tr>
		      						...
		      						<td colspan="2"  style="padding-top: 15px;">
										...
										<input type="text" name="SLIDE_IMAGE_NAME" style="display:none;"
											id="SLIDE_IMAGE_NAME" accept="image/*" maxlength="32"											
											placeholder="来选择任何文件" title="图片" style="width: 260px;"/>
		      						</td>
		      					</tr>
		      					<tr>
		      						<td colspan="2">
		  	...
		  	<div class="modal fade" id="viewSlidingImage" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
				<div class="modal-dialog" role="document" style="width: 800px">
					<div class="modal-content">
						<div class="modal-header">
							...
							<h3 class="modal-title" id="viewModalLabel" style="line-height: 1">画像</h3>
						</div>
						<div class="modal-body">
							<img id="VIEW_SLIDING_IMAGE" style="width:100%;"></img>											
						</div>
						<div class="modal-footer">										
							<button type="button" class="btn btn-app btn-primary btn-xs" data-dismiss="modal">取消</button>
						</div>
					</div>
...
<script>
	$("#simple-table").on('click', ".btn_m_view_img", function() {
		var node_row = $(this).parent().parent();
		var filePath = node_row.find(".sl-name").find("img").attr("src");
		filePath = filePath.replace("/thumb", "");
		
		$("#VIEW_SLIDING_IMAGE").attr("src", filePath);
	});
	...
	function search(){
		...
		$("#tourSlideForm").attr("action", "tour/listSlide/?COMMENT=" + DESC + "&STATE=" + STATE);
		$("#tourSlideForm").submit();
	}
	...
	function updateSlide(_id, _name, _desc, _vType, _vState){
		...
		$("#SLIDE_IMAGE_PREVIEW").attr('src', "<%=basePath%>uploadFiles/uploadImgs/tour/" + _name);
	...
	function delSlide(_id, _name){
		...
		bootbox.confirm(options, function(result) {
			if(result) {
				top.jzts();
				var url = "<%=basePath%>tour/deleteSlide?ID=" + _id;
	...
	function changeStateSlide(_id, _state){
		top.jzts();
		if(_state == "0")
			_state = "1";
		else
		...
		var url = "<%=basePath%>tour/changeStateSlide?ID=" + _id + "&STATE=" + _state;
		$.get(url, function(data){
			search();
			$(top.hangge());
		});

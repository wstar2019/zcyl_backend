...
<body>
	<div class="page-header">
		<ul class="navi">
			...
			<li><a target="mainFrame" href="fp/listDPRKSpecial">朝鲜特产</a></li>
		</ul>
	</div>

				<div class="page-content" style="padding: 10px;">
					<div class="container-fluid">
						<form action="fp/listFPSpecial" method="post" name="fpSpecialForm" id="fpSpecialForm">

							<table id="simple-table" class="table table-hover" style="margin-top: 5px;">
								<thead>
									<tr>
										<!-- 체크복스, 序号， 商品名称，销售数量，销售金额，浏览数 -->
								...
								<tbody>
									...
									<c:choose>										
										<c:when test="${not empty specialList}">
											<%
									            int numStds = 0;								
									            pageContext.setAttribute("numStds", numStds);
									        %>
											<c:forEach items="${specialList}" var="sp" varStatus="vs">
												<%
										            numStds++;								
										            pageContext.setAttribute("numStds", numStds);
										        %>
												<tr>
													<td class="center sp-check">
														<c:if test="${sp.VIEW_STATE == 1 }">
															<input type="checkbox" id="${sp.ID}" checked>
														</c:if>
														<c:if test="${sp.VIEW_STATE == 0 }">
															<input type="checkbox" id="${sp.ID}">
														</c:if>
														
													</td>
													<td class="center">${(page.currentPage -1) * page.showCount + numStds}</td>
													<td class="center sp-name">${sp.ART_NAME}</td>
													<td class="center sp-desc">${sp.SALE_QUANTITY}</td>
													<td class="center sp-desc">${sp.SALE_AMOUNT}</td>
													<td class="center sp-desc">${sp.VIEW_NUM}</td>
												</tr>
											</c:forEach>
										</c:when>
									</c:choose>
...
<script>
	...
	function updateState(_id, _state, _msg){
		...
		bootbox.confirm(options, function(result) {
			if(result) {
				top.jzts();
				var url = "<%=basePath%>fp/update_sp_vs?ID=" + _id + "&VIEW_STATE=" + _state;

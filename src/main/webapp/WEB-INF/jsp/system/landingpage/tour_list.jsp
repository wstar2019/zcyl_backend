...
<body>
	<div class="page-header">
		<ul class="navi">
			...
			<li><a target="mainFrame" href="fp/listDPRKTour">朝鲜旅游</a></li>
		</ul>
	</div>

				<div class="page-content" style="padding: 10px;">
					<div class="container-fluid">
						<form action="fp/listFPTour" method="post" name="fpTourForm" id="fpTourForm">
								...
								<tbody>
									...
									<c:choose>										
										<c:when test="${not empty tourList}">
									        ...
									        %>
											<c:forEach items="${tourList}" var="tour" varStatus="vs">
												<%
										            numStds++;								
										            pageContext.setAttribute("numStds", numStds);
										        %>
												<tr>
													<td class="center tour-check">
														<c:if test="${tour.VIEW_STATE == 1 }">
															<input type="checkbox" id="${tour.ID}" checked>
														</c:if>
														<c:if test="${tour.VIEW_STATE == 0 }">
															<input type="checkbox" id="${tour.ID}">
														</c:if>
														
													</td>
													<td class="center">${(page.currentPage -1) * page.showCount + numStds}</td>
													<td class="center tour-name">${tour.TOUR_NAME}</td>
													<td class="center tour-desc">${tour.SALE_QUANTITY}</td>
													<td class="center tour-desc">${tour.SALE_AMOUNT}</td>
													<td class="center tour-desc">${tour.VIEW_NUM}</td>
												</tr>
											</c:forEach>
										</c:when>
									</c:choose>
...
<script>
	...
	function updateState(_id, _state, _msg){
		...
		bootbox.confirm(options, function(result) {
			if(result) {
				top.jzts();
				var url = "<%=basePath%>fp/update_tour_vs?ID=" + _id + "&VIEW_STATE=" + _state;

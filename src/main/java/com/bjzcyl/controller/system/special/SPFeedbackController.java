...
public class SPFeedbackController extends BaseController {
	...
	String menuUrl = "sp/listFeedback";
	
	@Resource(name="spFeedbackService")
	private SPFeedbackManager spFeedbackService;
	...
	public ModelAndView listFeedback(Page page)throws Exception{				
		...
		try{
			feedbackList = spFeedbackService.listFeedback(page);		
		}catch(Exception e){
			logger.error(e.toString(), e);
		}
		mv.setViewName("system/special/feedback_list");
	...
	public Object deleteFeedback() throws Exception{
		...
		try{
			spFeedbackService.deleteFeedback(pd);
		}catch(Exception e){
			logger.error(e.toString(), e);
			errInfo = "error";
		}
